# Taken from https://github.com/miyosuda/unreal/blob/master/train/rmsprop_applier.py
# In A3C, multiple agents are updating the weights asynchronously, therefore, we need
# to update the optimizer function given by TensorFlow.
# In TensorFlow, the updates are done on the main network, since RMSProp will be
# applied on the local network (the networks of the agents), we need to
# grab the weights of the respective local networks and update the global network
# once we reach a terminal state or after t_max steps.
# RMSPropApplier is shared amongst all threads, therefore, momentum will
